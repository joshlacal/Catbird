...
// Source: app.bsky.feed.getActorLikes
import Foundation
import AppIntents
import Petrel

@available(iOS 18.0, *)
struct LikedPostsIntent: AppIntent {
    static let title: LocalizedStringResource = "Liked Posts"
    static let description = IntentDescription("Get the posts liked by a Bluesky account.")
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<[PostEntity]> & ProvidesDialog {
        ...
        let params = AppBskyFeedGetActorLikes.Parameters(actor: try ATIdentifier(string: actor), limit: limit.map { min(max($0, 1), 100) })

        let output = try unwrapIntentResponse(await client.app.bsky.feed.getActorLikes(input: params))
