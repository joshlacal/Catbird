...
//            }
    ...
    func unfollow(did: String) async throws -> Bool {
        ...
        do {
            ...
            let params = AppBskyActorGetProfile.Parameters(actor: did)
            ...
            guard profileCode == 200, 
                  ...
                  let followingUri = viewer.following else {
    ...
    func unblock(did: String) async throws -> Bool {
        ...
        do {
            ...
            guard profileCode == 200,
                  let profile = profile,
                  let viewer = profile.viewer,
                  let blockUri = viewer.blocking else {
    ...
    @discardableResult
    func refreshBlockCache() async throws -> Set<String> {
        // Ensure client exists
        guard let client = atProtoClient else {
...
//                }
    ...
    func mute(did: String) async throws -> Bool {
        ...
        do {
            let input = AppBskyGraphMuteActor.Input(actor: did)
    ...
    func unmute(did: String) async throws -> Bool {
        ...
        do {
            let input = AppBskyGraphUnmuteActor.Input(actor: did)
    ...
    @discardableResult
    func refreshMuteCache() async -> Set<String> {
        // Ensure client exists
        guard let client = atProtoClient else {
...
//                    }
    ...
    func refreshFollowingCache() async throws -> [String: ATProtocolURI] {
        ...
        do {
            ...
            repeat {
                let params = AppBskyGraphGetFollows.Parameters(actor: userDID, limit: 100, cursor: cursor)
...
//                    if let did = profile.did, let viewerState = profile.viewer, let following = viewerState.following {
                    allFollowing[profile.did] = profile.viewer?.following
//                    }
                }
...
extension AppState {
    ...
    @discardableResult
    func follow(did: String) async throws -> Bool {
        try await graphManager.follow(did: did)
    }
    
    /// Unfollows a user by their DID
    @discardableResult
    func unfollow(did: String) async throws -> Bool {
        try await graphManager.unfollow(did: did)
    }
    ...
    func refreshSocialGraph() async {
        ...
        Task {
            _ = await graphManager.refreshMuteCache()
        }
    }
}
