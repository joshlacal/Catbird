...
// MARK: - ⚠️ DEPRECATED STREAMING CAR PARSER ⚠️

/// Deprecated: Use `ATProtoRepoParser` instead, which correctly handles CAR binary format,
/// self-describing CID structure, and MST node traversal.
@available(*, deprecated, message: "Use ATProtoRepoParser instead")
final class StreamingCARParser {
    ...
    private let logger = Logger(subsystem: "blue.catbird", category: "StreamingCARParser")
    ...
    private func checkMemoryUsage() async throws {
        ...
        if memoryUsage > 1_000_000_000 { // 1GB critical (iOS kills at ~1.4GB)
            throw StreamingCARError.memoryLimitExceeded(memoryUsage)
        } else if memoryUsage > 600_000_000 { // 600MB warning — flush pressure
            logEssential("High memory usage: \(formatBytes(memoryUsage))")
