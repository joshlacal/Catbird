...
// MARK: - Repository Browser ViewModel

/// ViewModel for browsing parsed repository data
@MainActor
@Observable
final class RepositoryBrowserViewModel {
    ...
    private let logger = Logger(subsystem: "blue.catbird", category: "RepositoryBrowser")
...
struct RepositoryExportData: Codable {
    ...
    init(repository: RepositoryRecord, posts: [ParsedPost], profiles: [ParsedProfile], connections: [ParsedConnection], media: [ParsedMedia]) {
        ...
        self.warning = "This export contains parsing results. Accuracy and completeness are not guaranteed."
    }
}
