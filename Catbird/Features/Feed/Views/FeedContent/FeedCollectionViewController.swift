...
final class FeedCollectionViewController: UIViewController {
    ...
    /// Pull-to-refresh anchor captured during pull gesture
    ...
    @objc private func handleRefresh() {
        ...
        Task { @MainActor in
            // Use pre-captured anchor instead of capturing now (after bounce-back)
            let scrollAnchor = pullToRefreshAnchor ?? scrollTracker.captureScrollAnchor(collectionView: collectionView)
            
            // IMPORTANT: Capture the OLD post ID from the anchor index path NOW (before refresh)
            var anchorWithOldPostId: ScrollPositionTracker.ScrollAnchor? = nil
            if let anchor = scrollAnchor,
               ...
               anchor.indexPath.item < stateManager.posts.count {
                ...
                controllerLogger.debug("📎 SCROLL_DEBUG: Captured OLD post ID '\(oldPostId)' at anchor index \(anchor.indexPath.item)")
                
                // Create a new anchor that includes the old post ID
                ...
                anchorWithOldPostId = scrollAnchor
            }
            
            if let anchor = pullToRefreshAnchor {
                controllerLogger.debug("✅ SCROLL_DEBUG: Using pre-captured pull-to-refresh anchor with offsetY=\(anchor.offsetY)")
            } else {
                controllerLogger.debug("⚠️ SCROLL_DEBUG: No pre-captured anchor, falling back to current capture")
            }
            
            // Save scroll position before refresh starts
            saveScrollPositionForPersistence()
            
            // Use the anchor for position preservation
            await performAdvancedRefreshWithPositionPreservation(freshAnchor: anchorWithOldPostId)
            
            // Clear the pull-to-refresh anchor
    ...
    /// This is the sophisticated logic from the UIKitFeedView reference implementation
    @MainActor
    private func updateDataWithNewPostsAtTop(
        ...
        // Use the post ID from the anchor (captured before refresh), not from current posts
        let anchorPostId = originalAnchor.postId ?? (anchorIndexPath.item < stateManager.posts.count ? stateManager.posts[anchorIndexPath.item].id : "")
        ...
        collectionView.performBatchUpdates({ [weak self] in
            ...
            // Use standard anchor-based position preservation for all cases
            if let newAnchorAttributes = self.collectionView.layoutAttributesForItem(at: newAnchorIndexPath) {
                // Calculate based on anchor movement to preserve relative viewport position
                ...
                // Calculate new offset to restore the anchor to the same visual position in viewport
                let newCalculatedOffsetY = newAnchorY - originalAnchorVisibleY
                
                self.controllerLogger.debug("🎯 SCROLL_DEBUG: VIEWPORT PRESERVATION - originalAnchorVisibleY=\(originalAnchorVisibleY), should appear at same position")
    ...
    @MainActor
    private func performAdvancedRefreshWithPositionPreservation(freshAnchor: ScrollPositionTracker.ScrollAnchor?) async {
        ...
        // Use the fresh anchor captured at refresh trigger time
        let scrollAnchor = freshAnchor
        
        if let anchor = scrollAnchor {
            controllerLogger.debug("⚓ SCROLL_DEBUG: REFRESH - Using fresh anchor - indexPath=\(anchor.indexPath), offsetY=\(anchor.offsetY), itemFrameY=\(anchor.itemFrameY)")
        } else {
...
extension FeedCollectionViewController: UICollectionViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        // Capture anchor when user starts pulling to refresh
        ...
        if scrollView.contentOffset.y >= 0 && !refreshControl.isRefreshing {
            if pullToRefreshAnchor != nil {
                controllerLogger.debug("🔼 SCROLL_DEBUG: Cleared pull-to-refresh anchor - user cancelled pull")
                pullToRefreshAnchor = nil
            }
        }
